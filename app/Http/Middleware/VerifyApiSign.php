...
class VerifyApiSign
{
    ...
    protected function verifySign(Request $request)
    {
        ...
        $source = '';
        if(($request->is("api/auth/auto-login"))){
            $source = 'auto-login';
        }

        $sign = vss_paas_util()->sign($params, vss_config('paas.apps.lite.appSecret'), $source);
        if ($sign != $params['sign']) {
            throw new ValidationException(ResponseCode::AUTH_LOGIN_SIGN);
        }

        if (!isset($params['signed_at']) || (abs(time() - $params['signed_at']) > 60)) {
